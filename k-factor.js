...
function genGcode(settings) {
    ...
    var RANGE_K = K_END - K_START,
        ...
        PRINT_SIZE_X = (2 * LENGTH_SLOW) + LENGTH_FAST  + (USE_LINENO ? 8 : 0),
        CENTER_X = (NULL_CENTER ? 0 : BED_X / 2),
        CENTER_Y = (NULL_CENTER ? 0 : BED_Y / 2),
        PAT_START_X = CENTER_X - (0.5 * LENGTH_FAST) - LENGTH_SLOW - (USE_LINENO ? 4 : 0),
  ...
  var patSettings = {
    ...
    var k_script = '; ### Prusa Slicer K-Factor Calibration Pattern ###\n' +
                    '; -------------------------------------------------\n' +
                    ';\n' +
                    '; Printer: ' + PRINTER + '\n' +
                    '; Filament: ' + FILAMENT + '\n' +
                    ...
                    // TODO: check if prusa understand P + TOOL_INDEX or not
                    'M106 P' + TOOL_INDEX + ' S' + Math.round(FAN_SPEED * 2.55) + ' ; Start print fan\n';
  ...
  // Always print the frame, it helps with print removal from the bed and acts as a free prime
  ...
  k_script += ';\n' +
    ...
    // Tunr off pressure advance for the rest of the print
    k_script += ';\n' +
                'M117 K0\n' +
                'M900 K0 ; Set K-factor 0\n';

    // mark area of speed changes
    var refStartX1 = CENTER_X - (0.5 * LENGTH_FAST) - (USE_LINENO ? 4 : 0),
        refStartX2 = CENTER_X + (0.5 * LENGTH_FAST) - (USE_LINENO ? 4 : 0),
...
function doEfeed(dir, basicSettings) {
    if (dir === '+') {
        return 'G1 E' + basicSettings['retractDist'] + ' F' + basicSettings['unretractSpeed'] + ' ; un-retract\n';
    }
    else {
...
function createStdPattern(startX, startY, basicSettings, patSettings) {
    ...
    for (var i = patSettings['kStart']; i <= patSettings['kEnd']; i += patSettings['kStep']) {
        gcode += 'M900 K' + round10(i, -3) + ' ; set K-factor\n' +
                'M117 K' + round10(i, -3) + ' ; \n' +
                doEfeed('+', basicSettings) +
...
function setLocalStorage() {
  var K_START = parseFloat($('#K_START').val()),
      K_END = parseFloat($('#K_END').val()),
      K_STEP = parseFloat($('#K_STEP').val());
...
function validateInput() {
  ...
  if ((round10(parseFloat(testNaN['K_END']) - parseFloat(testNaN['K_START']), -3) * Math.pow(10, decimals)) % (parseFloat(testNaN['K_STEP']) * Math.pow(10, decimals)) !== 0) {
